...
class StepMetrics(BaseModel):
    ...
    input_tokens: int = Field(...,
                              description="Tokens sent to the LLM for "
                              "this step")
    output_tokens: int = Field(...,
                               description="Tokens generated by the LLM "
                               "for this step")
    request_time_ms: float = Field(
        ..., description="Wall-clock time for the LLM API call in "
        "milliseconds")
    ...
    llm_output: str = Field(
        default="", description="Raw text generated by the LLM before "
        "code extraction")
    ...
    retries: int = Field(
        default=0, description="Number of LLM API retries before getting "
        "a successful response (0 = first attempt succeeded)")
...
class SolutionOutput(BaseModel):
    ...
    total_input_tokens: int = Field(...,
                                    description="Sum of input_tokens across"
                                    " all steps")
    total_output_tokens: int = Field(...,
                                     description="Sum of output_tokens across"
                                     " all steps")
    total_time_seconds: float = Field(
        ..., description="Wall-clock time from agent start to finish")
    steps: List[StepMetrics] = Field(
        default_factory=list, description="Per-step metrics, one entry per"
        " agent iteration")
...
class SWEBenchTaskInput(BaseModel):
    ...
    problem_statement: str = Field(
        ..., description="The GitHub issue description, what needs "
        "to be fixed")
